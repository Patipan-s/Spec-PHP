...
function Insert(){
    ...
    // ถ้าทุกอย่างเรียบร้อยให้ insert ข้อมูลใน database
      ...
      $query = "INSERT INTO notebooktable(nname,img,cpu,ram,hdd,gpu,display) ";
      $query .= "VALUES ('$nname','$image','$cpu','$ram','$hdd','$gpu','$display')";
...
function Update() {
    if(isset($_POST['submit'])) {
        ...
        $id = $_POST['id'];
        // ลบรูปภาพของ id นั้น
        $sql = "SELECT img FROM notebooktable WHERE id = $id";
      ...
      // ถ้าอัพโหลดรูปสำเร็จให้ทำ update ข้อมูลใน database
        ...
        // เลือก row ข้อง id นั้น
          $query = "UPDATE notebooktable SET ";
          $query .= "nname = '$nname', ";
          $query .= "cpu = '$cpu', ";
          $query .= "ram = '$ram', ";
          $query .= "hdd = '$hdd', ";
...
function Delete(){
    ...
    if(isset($_POST['del_submit'])) {
        ...
        $query = "DELETE FROM `notebooktable` WHERE `id` = '$id'";
        $result = mysqli_query($conn, $query);
      } 
}
